...
def transform_user_data(raw: pd.DataFrame, filename: str = "Unknown") -> pd.DataFrame:
    ...
    # Force sample_data to be treated as business data
    ...
    if "titanic" in filename.lower() or "survived" in df.columns:
        ...
        # Use the first detected revenue-like column
        rev_source = df.get(rev_col, pd.Series([50000] * len(df)))
        df["revenue"] = pd.to_numeric(rev_source, errors="coerce").fillna(50000)
        df["customers"] = np.random.randint(80, 400, len(df))
