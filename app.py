"""
DecisionIQ — Final Clean & Improved Version
"""
import pandas as pd
...
with st.sidebar:
    ...
    st.caption("📈 Forecast : XGBoost / RandomForest")
    st.caption("🔁 Churn : RandomForest")
    st.caption("🚨 Anomaly : Z-Score")
    st.caption("🤖 Insights : Groq LLaMA-3.1 70B")
...
if run_btn or "results" not in st.session_state or st.session_state.get("last_filename") != (uploaded.name if uploaded else "sample_data.csv"):
    with st.spinner("Running full analysis..."):
        st.session_state.results = pipeline.run()
        st.session_state.last_filename = uploaded.name if uploaded else "sample_data.csv"
...
if not is_business_data:
    st.warning(f"⚠️ Dataset '{pipeline.filename}' is not a standard business dataset. Some values are synthetic.")
...
k1.metric("📈 Forecast Revenue (M+4)", f"₹{fp_val/100_000:.1f}L", f"{fp_d:+.1f}% vs prev")
...
k4.metric("🚨 Anomalies Detected", len(R["anomalies"]), f"{len([a for a in R['anomalies'] if a.get('Severity')=='High'])} high")
...
tab_fc, tab_ch, tab_an, tab_cmp, tab_wi, tab_ai = st.tabs([
    "📈 Forecast", "🔁 Churn", "🚨 Anomalies", "📊 Model Comparison", "🔮 What-If", "🤖 CEO Assistant"
])
...
with tab_fc:
    ...
    with c1:
        ...
        fig.add_trace(go.Scatter(x=months, y=R["lower"], mode="lines", fill="tonexty",
                                 fillcolor="rgba(59,130,246,0.15)", line=dict(width=0), name="95% CI"))
        fig.add_trace(go.Scatter(x=months, y=R["forecast_prev"], mode="lines+markers",
                                 name="Previous Scenario", line=dict(color="#4a5568", dash="dot")))
        fig.add_trace(go.Scatter(x=months, y=R["forecast"], mode="lines+markers",
                                 name="Forecast", line=dict(color="#3b82f6", width=3)))
        fig.update_layout(**L(320, yaxis=dict(tickprefix="₹", tickformat=".2s"),
                              legend=dict(orientation="h", yanchor="bottom", y=1.02)))
        st.plotly_chart(fig, use_container_width=True)
        st.caption(f"R² = {R['forecast_r2']:.3f} | Confidence Bands = ±1.5σ")
...
# ====================== CEO Assistant Tab ======================
with tab_ai:
    st.subheader("🤖 CEO Assistant — Friendly LLaMA-3.1 70B")
    
    SYS_PROMPT = f"""You are a friendly, professional, and helpful personal AI assistant to the CEO.
Dataset: {pipeline.filename} ({len(pipeline.df)} rows)
Business Suitable: {'Yes' if is_business_data else 'No'}
Current Metrics:
- 4-Month Revenue Forecast: {[f'₹{v/100000:.1f}L' for v in R.get('forecast', [])]}
- Current Churn Risk: {R['churn'].get('rate', 0):.1f}%
...
Be warm and natural. Use casual language for greetings. Keep answers concise but insightful."""

    if "chat" not in st.session_state:
        st.session_state.chat = [{"role": "assistant", "content": "👋 Hi! I'm your CEO Assistant. How can I help you today?"}]

    for msg in st.session_state.chat:
        with st.chat_message(msg["role"]):
            st.write(msg["content"])

    if prompt := st.chat_input("Ask anything about the business..."):
...
st.caption("DecisionIQ — All outputs are generated dynamically from your data • No hardcoded values")
